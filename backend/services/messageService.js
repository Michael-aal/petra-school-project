...
export const messageService = {
  listContacts: async (user, query = {}) => {
    ...
    const recipientRoles = ["principal", "admin", "super_admin", "superadmin", "teacher", "staff", "parent"].filter((role) => canMessage(senderRole, role));
    if (!recipientRoles.length) return { contacts: [] };
    const where = { schoolId, id: { not: user.id }, role: { in: recipientRoles }, ...(search ? { OR: [{ fullName: { contains: search, mode: "insensitive" } }, { email: { contains: search, mode: "insensitive" } }] } : {}) };
    const contacts = await prisma.user.findMany({ where, select: userSelect, orderBy: [{ fullName: "asc" }, { email: "asc" }], take: limit });
    return { contacts };
  },
  ...
  listMessages: async (user, query = {}) => {
    ...
    const search = query.search ? String(query.search).trim() : "";
    const where = { schoolId, ...(folder === "sent" ? { senderId: user.id } : { recipientId: user.id }) };
    if (search) where.OR = [{ subject: { contains: search, mode: "insensitive" } }, { body: { contains: search, mode: "insensitive" } }];
    const [total, messages] = await Promise.all([
      prisma.message.count({ where }),
      prisma.message.findMany({ where, orderBy: { sentAt: "desc" }, skip: (page - 1) * limit, take: limit, include: { sender: { select: userSelect }, recipient: { select: userSelect } } }),
    ]);
    return { messages, pagination: { page, limit, total, totalPages: Math.max(1, Math.ceil(total / limit)) }, folder };
  },

  sendMessage: async (user, payload) => {
    const schoolId = normalizeSchoolId(user);
    if (!payload.recipientId) { const err = new Error("Recipient is required"); err.statusCode = 400; throw err; }
    ...
    const notification = await prisma.notification.create({ data: { schoolId, userId: recipient.id, title: "New message", body: payload.subject ? String(payload.subject).trim() : "You received a new message." } });
    void pushNotificationService.sendToUser(recipient.id, { title: "New message", body: payload.subject ? String(payload.subject).trim() : "You received a new message.", notificationId: notification.id, url: "/dashboard/communication/messages", tag: `message-${notification.id}` }).catch(() => {});
    return message;
  },

  getConversation: async (user, otherUserId) => {
    const schoolId = normalizeSchoolId(user);
    if (!otherUserId) { const err = new Error("Conversation user ID is required"); err.statusCode = 400; throw err; }
    const otherUser = await prisma.user.findFirst({ where: { id: otherUserId, schoolId }, select: { id: true, role: true } });
    if (!otherUser || !canMessage(user.role, otherUser.role) && !canMessage(otherUser.role, user.role)) { const err = new Error("Conversation not accessible"); err.statusCode = 403; throw err; }
    return prisma.message.findMany({ where: { schoolId, OR: [{ senderId: user.id, recipientId: otherUserId }, { senderId: otherUserId, recipientId: user.id }] }, orderBy: { sentAt: "desc" }, include: { sender: { select: userSelect }, recipient: { select: userSelect } } });
  },
};
