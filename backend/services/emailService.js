...
const transportConfigAvailable = Boolean(
  process.env.SMTP_HOST &&
  process.env.SMTP_PORT &&
  process.env.SMTP_USER &&
  process.env.SMTP_PASS
);
const resendConfigAvailable = Boolean(
  process.env.RESEND_API_KEY &&
  process.env.FROM_EMAIL
);
const emailProviderAvailable =
  transportConfigAvailable || resendConfigAvailable;
const emailProvider = resendConfigAvailable
  ? "resend"
  : transportConfigAvailable
    ? "smtp"
    : "none";
...
const resend = resendConfigAvailable
...
const getFromEmail = (school, fromEmail) => {
  ...
  // Do not invent a sender address for Resend. A sender/domain must be
  // explicitly configured and verified with the provider.
  if (resend) {
    throw new Error(
      "FROM_EMAIL is required when Resend is configured"
    );
  }

  const website = String(school?.website || "")
    .trim()
    .replace(/^https?:\/\//, "")
    .split("/")[0];
...
export const getEmailProviderStatus = () => ({
  ...
  configured: emailProviderAvailable,
  smtpConfigured: transportConfigAvailable,
  resendConfigured: resendConfigAvailable,
  fromEmailConfigured: Boolean(process.env.FROM_EMAIL),
});

export const buildAdmissionEmailPayload = ({
  school,
  studentName,
  admissionCode,
  paymentUrl,
  ...
  const logo = school?.logo
    ? `<img src="${safeHtml(school.logo)}" alt="${safeHtml(school.name)}" style="max-height:56px;display:block;margin-bottom:16px"/>`
    : "";
...
export const buildAdmissionFailureEmailPayload = ({
  school,
  studentName,
  score,
  percentage,
...
const deliverAdmissionEmail = async ({
  school,
  admission,
  subject,
  html,
  ...
  const { log, deduped } = await createEmailLog({
    school,
    admission,
    subject,
    text,
  ...
  if (deduped) {
    return { success: true, reason: "deduped", log, provider: emailProvider };
  }
  ...
  try {
    const info = await sendEmailProvider({
      from: getFromEmail(school, fromEmail),
      to: sendTo,
      subject,
      text,
      html,
...
export const sendAdmissionEmail = async ({
  ...
  const { subject, html, text } = buildAdmissionEmailPayload({
    school,
    studentName,
    admissionCode,
    paymentUrl,
...
export const sendAdmissionFailureEmail = async ({
  ...
  const { subject, html, text } = buildAdmissionFailureEmailPayload({
    school,
    studentName,
    score,
    percentage,
