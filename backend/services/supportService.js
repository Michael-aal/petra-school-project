...
const mapTicket = (r) => ({
  id:r.id, schoolId:r.schoolId, subject:r.subject, description:r.description, category:r.category,
  priority:r.priority, status:r.status, assignedToId:r.assignedToId, createdAt:r.createdAt, updatedAt:r.updatedAt, resolvedAt:r.resolvedAt,
  createdBy:r.createdById ? { id:r.createdById, fullName:r.createdByName, email:r.createdByEmail, role:r.createdByRole, schoolId:r.createdBySchoolId } : null,
  assignedTo:r.assignedToId ? { id:r.assignedToId, fullName:r.assignedToName, email:r.assignedToEmail, role:r.assignedToRole } : null,
});
...
export const supportService = {
  listTickets: async (user, query = {}) => {
    ...
    if (isPlatform(user)) {
      ...
      if (!isSchoolAdmin(user)) filters.push(`t."createdById"=${add(user.id)}`);
    }
    ...
    const [rows, countRows] = await Promise.all([
      prisma.$queryRawUnsafe(`SELECT ${selectSql} FROM "SupportTicket" t LEFT JOIN "User" creator ON creator.id=t."createdById" LEFT JOIN "User" assignee ON assignee.id=t."assignedToId" ${where} ORDER BY t."updatedAt" DESC LIMIT ${limitParam} OFFSET ${offsetParam}`, ...values),
      prisma.$queryRawUnsafe(`SELECT COUNT(*)::int AS count FROM "SupportTicket" t ${where}`, ...filterValues),
    ]);
  ...
  getTicket: async (user, ticketId) => {
    ...
    if (!isPlatform(user)) {
      const schoolId=schoolIdFor(user); values.push(schoolId); access += ` AND t."schoolId"=$${values.length}`;
      if (!isSchoolAdmin(user)) { values.push(user.id); access += ` AND t."createdById"=$${values.length}`; }
    }
    const rows=await prisma.$queryRawUnsafe(`SELECT ${selectSql} FROM "SupportTicket" t LEFT JOIN "User" creator ON creator.id=t."createdById" LEFT JOIN "User" assignee ON assignee.id=t."assignedToId" WHERE ${access}`, ...values);
  ...
  addMessage: async (user,ticketId,payload) => {
    ...
    if (!body) { const e=new Error("Message body is required"); e.statusCode=400; throw e; }
    const id=crypto.randomUUID();
    const internal=Boolean(payload.isInternal)&&isPlatform(user);
    await prisma.$executeRawUnsafe(`INSERT INTO "SupportTicketMessage" ("id","ticketId","authorId","body","isInternal","createdAt") VALUES ($1,$2,$3,$4,$5,CURRENT_TIMESTAMP)`,id,ticketId,user.id,body,internal);
  ...
  updateTicket: async (user,ticketId,payload) => {
    ...
    await prisma.$executeRawUnsafe(`UPDATE "SupportTicket" SET "status"=$1,"priority"=$2,"assignedToId"=$3,"resolvedAt"=CASE WHEN $1 IN ('Resolved','Closed') THEN COALESCE("resolvedAt",CURRENT_TIMESTAMP) ELSE NULL END,"updatedAt"=CURRENT_TIMESTAMP WHERE id=$4`,status,priority,assignedToId,ticketId);
    return supportService.getTicket(user,ticketId);
  },
};
