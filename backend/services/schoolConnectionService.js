...
const schoolIdOf = (user) => {
  ...
  const schoolId = candidates.map(Number).find((value) => Number.isInteger(value) && value > 0);
  if (!schoolId) {
...
const assertSchoolRecords = async (schoolId, ids) => {
  const [teacher, classRecord, subject] = await Promise.all([
    ids.teacherId ? prisma.teacher.findFirst({ where: { id: ids.teacherId, schoolId } }) : null,
    ids.classId ? prisma.class.findFirst({ where: { id: ids.classId, schoolId } }) : null,
    ids.subjectId ? prisma.subject.findFirst({ where: { id: ids.subjectId, schoolId } }) : null,
  ]);
  ...
  if (ids.classId && !classRecord)
    throw Object.assign(new Error("Class not found in this school"), { statusCode: 404 });
  if (ids.subjectId && !subject)
    throw Object.assign(new Error("Subject not found in this school"), { statusCode: 404 });
};
...
export const schoolConnectionService = {
  listTeacherAssignments: async (user) => {
    ...
    return prisma.teacherClass.findMany({
      where: { schoolId },
      include: { teacher: { include: { user: { select: { id: true, fullName: true, email: true } } } }, class: true },
  ...
  removeTeacherClass: async (user, id) => {
    const schoolId = schoolIdOf(user);
    const assignment = await prisma.teacherClass.findFirst({ where: { id, schoolId } });
    if (!assignment)
      throw Object.assign(new Error("Teacher class assignment not found"), { statusCode: 404 });
  ...
  assignTeacherSubject: async (user, payload) => {
    ...
    return prisma.teacherSubject.upsert({
      ...
      update: {},
      include: { teacher: { include: { user: { select: { id: true, fullName: true } } } }, subject: true },
    });
  },
  ...
  assignClassSubject: async (user, payload) => {
    ...
    return prisma.subjectClass.upsert({ where: { classId_subjectId: { classId, subjectId } }, create: { classId, subjectId }, update: {}, include: { class: true, subject: true } });
  },

  removeClassSubject: async (user, id) => {
    const schoolId = schoolIdOf(user);
    const link = await prisma.subjectClass.findFirst({ where: { id, class: { schoolId } } });
    if (!link)
      throw Object.assign(new Error("Class subject assignment not found"), { statusCode: 404 });
  ...
  listReportCards: async (user, query = {}) => {
    ...
    return prisma.reportCard.findMany({ where, include: { student: { select: { id: true, name: true, admissionNumber: true, className: true } } }, orderBy: { createdAt: "desc" } });
  },
  ...
  publishReportCard: async (user, id, fileUrl) => {
    ...
    if (!normalizedUrl)
      throw Object.assign(new Error("fileUrl is required to publish a report card"), { statusCode: 400 });
    return prisma.reportCard.update({ where: { id }, data: { fileUrl: normalizedUrl } });
  },
};
