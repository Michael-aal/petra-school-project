...
export default function Register({ rolePreset = "" }) {
  ...
  const handleChange = (event) => {
    ...
    setUserInfo((current) => {
      ...
      if (name === "email") nextUser.email = value;
      if (name === "phone") nextUser.phoneNumber = value;
      if (name === "institution") nextUser.institution = value;
      if (name === "state") nextUser.state = value;
      if (name === "city") nextUser.city = value;
      if (name === "role") nextUser.role = value;
  ...
  const persistRegistrationUser = (userData) => {
    ...
    const nextUser = normalizeUser({
      ...
      phoneNumber: userData?.phoneNumber || userData?.phone || userInfo?.phoneNumber || userInfo?.phone || "",
  ...
  if (checkingSession) {
    return <main className="auth-page auth-page-loading"><LoaderCircle className="auth-spinner" size={34} /></main>;
  }

  return (
    <AuthShell
      variant={rolePreset === "principal" ? "admin-register" : ""}
      eyebrow="Create your account"
      subtitle={rolePreset === "principal" ? "Create your school administration account and start managing your institution." : "Set up your account to access the school dashboard, modules, and secure tools."}
    ...
    >
      <form className="auth-form" onSubmit={handleSubmit}>
        ...
        <label className="auth-field"><span>Phone Number</span><div className="auth-input-wrap"><Mail size={18} /><input name="phone" type="tel" placeholder="+234..." value={form.phone} onChange={handleChange} autoComplete="tel" /></div></label>

        {rolePreset ? (
          <div className="auth-field"><span>Registration Type</span><div className="auth-input-wrap" style={{ justifyContent: "space-between", padding: "0 14px" }}><span>{rolePreset === "principal" ? "School Administrator" : "Staff / Teacher"}</span><span style={{ color: "var(--primary)", fontWeight: 700, fontSize: "0.9rem" }}>Selected</span></div></div>
        ) : (
          <label className="auth-field"><span>Register As</span><div className="auth-input-wrap"><UserRound size={18} /><select name="role" value={form.role} onChange={handleChange}><option value="">Select Role</option><option value="student">Student</option><option value="teacher">Teacher</option><option value="parent">Parent</option><option value="principal">School Administrator</option></select></div>{errors.role ? <small>{errors.role}</small> : null}</label>
        )}
        ...
        <label className="auth-field"><span>How did you hear about us?</span><div className="auth-input-wrap"><input name="hearAbout" type="text" placeholder="e.g. LinkedIn, Friend, Event" value={form.hearAbout} onChange={handleChange} /></div></label>
        <label className="auth-field"><span>Email Address</span><div className="auth-input-wrap"><Mail size={18} /><input name="email" type="email" placeholder="name@example.com" value={form.email} onChange={handleChange} autoComplete="email" /></div>{errors.email ? <small>{errors.email}</small> : null}</label>

        <label className="auth-field"><span>Password</span><div className="auth-input-wrap"><Lock size={18} /><input name="password" type={showPassword ? "text" : "password"} placeholder="Create a password" value={form.password} onChange={handleChange} autoComplete="new-password" /><button type="button" className="auth-eye-btn" onClick={() => setShowPassword((current) => !current)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>{errors.password ? <small>{errors.password}</small> : null}</label>
        <label className="auth-field"><span>Confirm Password</span><div className="auth-input-wrap"><Lock size={18} /><input name="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="Confirm your password" value={form.confirmPassword} onChange={handleChange} autoComplete="new-password" /><button type="button" className="auth-eye-btn" onClick={() => setShowConfirmPassword((current) => !current)} aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"}>{showConfirmPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>{errors.confirmPassword ? <small>{errors.confirmPassword}</small> : null}</label>
        ...
        <p className="auth-switch">Already have an account? <Link to="/signin">Login</Link></p>
      </form>
    </AuthShell>
  );
}
