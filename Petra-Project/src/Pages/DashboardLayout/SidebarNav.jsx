...
import { LayoutDashboard, School, BookOpen, GraduationCap, Users, Bus, ClipboardCheck, FileText, BarChart2, UserCog, CreditCard, Receipt, Wallet, TrendingUp, Bell, Megaphone, HelpCircle, MessageSquare, Settings, Download, ChevronDown, ChevronRight, Calendar, ClipboardList, UserCheck, PlusCircle, X, LogOut, ShieldCheck, Sparkles, Rocket } from "lucide-react";
...
const navGroups = [
  ...
  { label:"Staff", icon:UserCog, children:[{label:"Teachers",icon:UserCog,href:"/dashboard/staff/teachers"},{label:"Admins",icon:UserCog,href:"/dashboard/staff/admins"},{label:"Staff Management",icon:UserCog,href:"/dashboard/staff/management"},{label:"Attendance",icon:ClipboardCheck,href:"/dashboard/staff/attendance"}] },
...
export function SidebarNav({onNavigate,collapsed=false,onClose}) {
  ...
  const normalizedRole=String(userInfo?.role||"").toLowerCase().replace(/\s+/g,"_");
  ...
  const [openGroups,setOpenGroups]=useState(()=>{const initial={}; navItems.forEach((group)=>{if(group.children?.some((child)=>location.pathname.startsWith(child.href)))initial[group.label]=true;}); return initial;});
  const toggleGroup=(label)=>setOpenGroups((prev)=>({...prev,[label]:!prev[label]}));
  const isActive=(href)=>href==="/dashboard"||href==="/"?location.pathname===href:location.pathname.startsWith(href);
