...
function StaffManagementWorkspace() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await authApi.createStaffInvitation({ ...form, assignedSubjects: splitSubjects(form.assignedSubjects) });
      setMessage("Teacher invitation created. Give the generated registration code to the teacher.");
      setForm(initialForm); setShowForm(false); await loadInvitations();
    } catch (err) {
      setError(err.data?.message || err.message || "Failed to create teacher invitation");
    } finally { setSubmitting(false); }
  };

  const copyCode = async (code) => { await navigator.clipboard.writeText(code); setMessage("Registration code copied to clipboard."); };

  const regenerateCode = async (registrationCode) => {
    setError("");
    try { await authApi.regenerateStaffInvitationCode({ registrationCode }); setMessage("Registration code regenerated."); await loadInvitations(); }
    catch (err) { setError(err.data?.message || err.message || "Failed to regenerate code"); }
  };

  const revokeCode = async (registrationCode) => {
    setError("");
    try { await authApi.revokeStaffInvitation({ registrationCode }); setMessage("Registration code revoked."); await loadInvitations(); }
  ...
  return (
    <div className="dashboard-home">
      ...
      <section className="dashboard-home-summary">
        ...
        <article className="dashboard-home-summary-card"><div className="dashboard-home-summary-top"><div><span>Revoked</span><strong>{summary.revoked}</strong></div><div className="dashboard-home-summary-icon tone-rose"><UserCog size={18} /></div></div></article>
      </section>
      <section className="dashboard-home-panel" style={{ marginBottom: 18 }}>
        ...
        {showForm ? <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12, marginTop: 16, padding: 16, border: "1px solid var(--app-border)", borderRadius: 16, background: "var(--app-surface)" }}><div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}><input name="staffName" placeholder="Teacher Full Name" value={form.staffName} onChange={handleChange} required /><input name="role" placeholder="Position / Role" value={form.role} onChange={handleChange} /><input name="department" placeholder="Department (Optional)" value={form.department} onChange={handleChange} /><input name="assignedClass" placeholder="Class Assigned (Optional)" value={form.assignedClass} onChange={handleChange} /><input name="assignedSubjects" placeholder="Subjects Assigned, comma separated (Optional)" value={form.assignedSubjects} onChange={handleChange} /><select name="employmentStatus" value={form.employmentStatus} onChange={handleChange}><option value="active">Active</option><option value="inactive">Inactive</option></select></div><div style={{ display: "flex", gap: 10, justifyContent: "flex-end", flexWrap: "wrap" }}><button type="button" onClick={() => setShowForm(false)}>Cancel</button><button type="submit" disabled={submitting}>{submitting ? "Generating..." : "Generate Teacher Code"}</button></div></form> : null}
