...
import { getParentPortalProfiles } from "../../../../utils/parentPortalData";
import "./PortalLinksPage.css";

function mapPortalProfileToRow(profile) {
  return {
    id: profile.id,
    parentName: profile.parentName,
    email: profile.email,
    linkedStudents:
      profile.linkedStudents ||
      (Array.isArray(profile.assignedStudents)
        ? profile.assignedStudents.length
        : 0),
    inviteStatus: profile.inviteStatus || "Pending",
    lastInvited: profile.lastInvited || "Just now",
  };
}

const initialParents = getParentPortalProfiles().map(mapPortalProfileToRow);
...
export default function PortalLinksPage() {
  ...
  const activeAccounts = parents.filter(
    (p) => p.inviteStatus === "Active",
  ).length;
  const pendingInvites = parents.filter(
    (p) => p.inviteStatus === "Pending",
  ).length;

  // Handle Copy Link
  const handleCopyLink = (parent) => {
    const loginUrl = `${window.location.origin}/portal/dashboard/${parent.id}`;
      ...
      <div className="pl-header">
        <div className="pl-header-left">
          ...
          <div>
            ...
            <p>
              Manage invitations and distribute secure login links to parents.
            </p>
          </div>
        </div>
      </div>
            ...
            {filteredParents.map((parent) => (
              <div key={parent.id} className="pl-list-item">
                ...
                <div className="pl-item-left">
                  ...
                  <div className="pl-info">
                    ...
                    <span className="pl-wards-badge">
                      <Users size={12} /> {parent.linkedStudents} Ward
                      {parent.linkedStudents !== 1 ? "s" : ""}
                    </span>
                  </div>
                </div>
                ...
                <div className="pl-item-center">
                  <span
                    className={`pl-status-badge ${parent.inviteStatus.toLowerCase()}`}
                  >
                    {parent.inviteStatus === "Active" ? (
                ...
                <div className="pl-item-right">
                  ...
                  >
                    ...
                    <span>
                      {copiedId === parent.id ? "Copied!" : "Copy Link"}
                    </span>
                  </button>

                  <button
                    className="pl-btn pl-btn-primary"
                    onClick={() => navigate(`/portal/dashboard/${parent.id}`)}
                  >
                    <Eye size={16} />
