  ...
  return (
    <article className="results-card">
      ...
      <div className="results-grid">
        ...
        <div><span>Marks Obtained</span><strong>{formatNumber(item.score ?? item.marks)}</strong></div>
        <div><span>Total Marks</span><strong>{formatNumber(item.totalMarks)}</strong></div>
        <div><span>Percentage</span><strong>{item.percentage != null ? `${Number(item.percentage).toFixed(2)}%` : "N/A"}</strong></div>
        <div><span>Grade</span><strong>{item.grade || "N/A"}</strong></div>
        <div><span>Pass / Fail</span><strong>{isPending ? "Pending" : isPass ? "Pass" : isFail ? "Fail" : "Not assessed"}</strong></div>
        <div><span>Attempt Status</span><strong>{item.attemptStatus || "Completed"}</strong></div>
        <div><span>Result State</span><strong>{isPending ? "Attempt recorded; final result pending" : "Final result available"}</strong></div>
        <div><span>Completed</span><strong>{formatDateTime(item.completedAt)}</strong></div>
        <div><span>Admission / School Code</span><strong>{item.schoolCode || item.admissionCode || item.applicationCode || "Not available"}</strong></div>
        <div><span>Admission Status</span><strong>{item.admissionStatus || "Not available"}</strong></div>
        <div><span>Application Code</span><strong>{item.applicationCode || "Not available"}</strong></div>
        <div><span>Applicant</span><strong>{item.applicantName || item.studentName || "Unknown Student"}</strong></div>
        <div><span>Applicant ID</span><strong>{item.applicantId || "Not available"}</strong></div>
        <div><span>Exam Reference</span><strong>{item.examReference || "Not available"}</strong></div>
        <div><span>Attempt ID</span><strong>{item.attemptId || "Not available"}</strong></div>
        <div><span>Result ID</span><strong>{item.resultId || "Pending"}</strong></div>
      </div>
    </article>
  );
};
    ...
    <div className="dashboard-page results-page">
      ...
      {!loading && !error && pagination.totalPages > 1 && (
        <div className="results-pagination">
          <button type="button" className="results-page-button" onClick={() => goToPage(pagination.page - 1)} disabled={pagination.page <= 1}>Previous</button>
          <span>Page {pagination.page} of {pagination.totalPages}</span>
          <button type="button" className="results-page-button" onClick={() => goToPage(pagination.page + 1)} disabled={pagination.page >= pagination.totalPages}>Next</button>
        </div>
      )}
    </div>
  );
