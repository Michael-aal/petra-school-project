...
const resolveApiBaseUrl = () => {
  if (!normalizedApiUrl || typeof window === "undefined") return normalizedApiUrl;
...
const persistTabCredentials = (response) => {
  ...
  if (response?.tabSession?.accessToken) {
    storage.setItem(TAB_ACCESS_KEY, response.tabSession.accessToken);
  }
  if (response?.tabSession?.refreshToken) {
    storage.setItem(TAB_REFRESH_KEY, response.tabSession.refreshToken);
  }
...
async function request(path, options = {}, { tabCredential = true, retryAuth = true } = {}) {
  ...
  if (
    tabCredential &&
    isTabAuthMode() &&
    !headers["X-Petra-Tab-Id"] &&
    !headers["x-petra-tab-id"]
  ) {
    headers["X-Petra-Tab-Id"] = ensureTabId();
  }
  ...
  if (!response.ok) {
    if (
      ...
      try {
        ...
        clearTabCredentials();
        if (retryAuth) {
          return request(path, options, { tabCredential: false, retryAuth: false });
        }
      }
    }
...
export const authApi = {
  ...
  staffInvitationDetails: (token) =>
    request(`/api/auth/staff/invitations/${encodeURIComponent(token)}`, { method: "GET" }),
  ...
  regenerateStaffInvitationCode: (payload) =>
    protectedPost("/api/auth/staff/invitations/regenerate", payload),
  managedTeachers: () => request("/api/auth/staff/teachers", { method: "GET", cache: "no-store" }),
  deactivateTeacher: (teacherUserId) =>
    request(`/api/auth/staff/teachers/${encodeURIComponent(teacherUserId)}/deactivate`, { method: "PATCH" }),
  reactivateTeacher: (teacherUserId) =>
    request(`/api/auth/staff/teachers/${encodeURIComponent(teacherUserId)}/reactivate`, { method: "PATCH" }),
