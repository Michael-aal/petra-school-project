...
export const financeApi = {
  publicStudentLookup: (studentCode) => {
    const query = new URLSearchParams({ studentCode }).toString();
    return request(`/api/finance/public/lookup?${query}`);
  },
  publicPayment: (payload) => request("/api/finance/public/payments", {
    method: "POST",
    body: JSON.stringify(payload),
    headers: { "x-idempotency-key": globalThis.crypto?.randomUUID?.() || `public-${Date.now()}-${Math.random().toString(16).slice(2)}` },
  }),
  payments: (params = {}) => {
